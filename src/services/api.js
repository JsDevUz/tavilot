...
export const API_BASE_URL = 'http://38.242.217.124:5001';
...
export function normalizeEnvelope(payload, status = 200) {
  if (payload && typeof payload === 'object' && 'ok' in payload) {
    ...
    throw new ApiError(payload.detail || 'Request failed', status, payload.error_code, payload);
  }
  return payload;
}
...
export async function apiRequest(path, options = {}) {
  ...
  if (fetcher !== fetch) {
    ...
    if (!response.ok) {
      throw new ApiError(payload.detail || payload.message || 'Request failed', response.status, payload.error_code, payload);
    }
  ...
  try {
    ...
    throw new ApiError(payload?.detail || error.message || 'Request failed', status, payload?.error_code, payload);
  }
}
